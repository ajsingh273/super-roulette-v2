...
def feature_engineering(spins):
    ...
    # Count of each number from 1–36 (36 features)
    for i in range(1, 37):
        features[f'count_{i}'] = np.count_nonzero(spins_array == i)

    # Ratios for each dozen (3 features)
    total = len(spins)
    features['dozen_1_ratio'] = sum(1 <= s <= 12 for s in spins) / total
    features['dozen_2_ratio'] = sum(13 <= s <= 24 for s in spins) / total
    features['dozen_3_ratio'] = sum(25 <= s <= 36 for s in spins) / total

    # Recent weighted momentum for each dozen (3 features)
    ...
    features['std_last_5'] = np.std(spins[-5:])
    features['std_last_10'] = np.std(spins[-10:])
    features['max_last_10'] = np.max(spins[-10:])
    features['min_last_10'] = np.min(spins[-10:])
